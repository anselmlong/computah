...
class ReleaseTests(unittest.TestCase):
    ...
    def test_staged_site_matches_archive_and_leaves_source_untouched(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            (source / "README.md").write_text("Private development notes")
            (source / ".env").write_text("TEST_SECRET=not-public")
            (source / "docs.html").write_text("Public setup guide")
            (source / "assets").mkdir()
            (source / "assets/icon.svg").write_text("Public logo")
            (source / "assets/.DS_Store").write_text("Hidden metadata")
            ...
            self.assertFalse((root / "stage/README.md").exists())
            self.assertFalse((root / "stage/.env").exists())
            self.assertFalse((root / "stage/assets/.DS_Store").exists())
            self.assertEqual((root / "stage/docs.html").read_text(), "Public setup guide")
            self.assertTrue((root / "stage/assets/icon.svg").exists())
